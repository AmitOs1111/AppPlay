...
export function MakingNote({ setAddNote }) {
  ...
  const [toggleTypeNote, setToggleTypeNote] = useState(false)
  ...
  function onToggleTypeNote() {
    setToggleTypeNote((prevToggle) => !prevToggle)
  }
    ...
    >
      {!isOpenMakingNote && (
        <div className="box-show-making-note flex space-between">
          ...
          <div className={`type-note ${toggleTypeNote ? 'open' : ''}`}>
          ...
          <div
            onClick={() => onToggleTypeNote()}
            className="btn-hamburger-type-note"
          >
